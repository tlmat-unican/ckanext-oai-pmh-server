# Copied from kangmoesss/ckanext-oaipmh-1 GitHub repository
...
rdf_reader = MetadataReader(
    fields={
        ...
        "description": (
            "textList",
            "rdf:RDF/ow:Publication/dc:description/text()",
        ),
        "publisher": ("textList", "rdf:RDF/ow:Publication/dc:publisher/text()"),
        "contributor": (
            "textList",
            "rdf:RDF/ow:Publication/dc:contributor/text()",
        ),
        ...
        "identifier": (
            "textList",
            "rdf:RDF/ow:Publication/dc:identifier/text()",
        ),
    ...
    namespaces={"rdf": NSRDF, "ow": NSOW, "dc": NS_DC},
)
...
def dcat2rdf_writer(element, metadata):
    ...
    e_dc = etree.fromstring(str.encode(metadata, "utf-8"))
    # e_dc = etree.XML(metadata)
    element.append(e_dc)
